  ...
  return (
    <section className="relative">
      <div className="relative h-[500px] md:h-[600px] overflow-hidden">
        ...
        {/* CONTENT (Properly over image) */}
        <div className="absolute inset-0 flex items-center z-20">
          <div className="container mx-auto px-4">
            <div className="max-w-xl text-white">
              <span className="inline-block px-4 py-1 bg-primary/20 rounded-full text-sm mb-4 backdrop-blur-sm border border-primary/30">
                New Season Collection
              </span>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3 z-30">
            {bannerList.map((_, idx) => (
              <button
                key={idx}
                onClick={() => setCurrentIndex(idx)}
                className={`w-3 h-3 rounded-full transition ${idx === currentIndex ? 'bg-primary scale-110' : 'bg-white/50'
                  }`}
              />
            ))}
          </div>
